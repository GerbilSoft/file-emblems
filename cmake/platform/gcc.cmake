...
SET(FE_C_FLAGS_COMMON "")
SET(FE_CXX_FLAGS_COMMON "")
SET(FE_EXE_LINKER_FLAGS_COMMON "")
...
FOREACH(FLAG_TEST ${CFLAGS_WARNINGS} ${CFLAGS_WERROR_FORMAT} ${CFLAGS_OPTIONS})
	...
	IF(CFLAG_${FLAG_TEST_VARNAME})
		SET(FE_C_FLAGS_COMMON "${FE_C_FLAGS_COMMON} ${FLAG_TEST}")
	...
	IF(CXXFLAG_${FLAG_TEST_VARNAME})
		SET(FE_CXX_FLAGS_COMMON "${FE_CXX_FLAGS_COMMON} ${FLAG_TEST}")
...
IF(NOT CMAKE_COMPILER_IS_GNUCC OR (CMAKE_CXX_COMPILER_VERSION VERSION_GREATER 9.1))
	...
	IF(CXXFLAG_SUGGEST_OVERRIDE)
		SET(FE_CXX_FLAGS_COMMON "${FE_CXX_FLAGS_COMMON} -Wsuggest-override -Wno-error=suggest-override")
...
FOREACH(FLAG_TEST "--sort-common" "--as-needed" "--build-id" "-Bsymbolic-functions" "--no-undefined" "--no-allow-shlib-undefined")
	...
	IF(LDFLAG_${FLAG_TEST})
		SET(FE_EXE_LINKER_FLAGS_COMMON "${FE_EXE_LINKER_FLAGS_COMMON} -Wl,${FLAG_TEST}")
...
SET(FLAG_TEST "-O")
	...
	IF(LDFLAG_${FLAG_TEST})
		SET(FE_EXE_LINKER_FLAGS_COMMON "${FE_EXE_LINKER_FLAGS_COMMON} -Wl,-O1")
	ENDIF(LDFLAG_${FLAG_TEST})
UNSET(FLAG_TEST)
...
SET(FLAG_TEST "--compress-debug-sections")
	...
	IF(LDFLAG_${FLAG_TEST})
		SET(FE_EXE_LINKER_FLAGS_COMMON "${FE_EXE_LINKER_FLAGS_COMMON} -Wl,${FLAG_TEST}=zlib")
	ENDIF(LDFLAG_${FLAG_TEST})
UNSET(FLAG_TEST)
...
IF(UNIX AND NOT APPLE)
	...
	IF(HAVE_DT_RELR)
		SET(FE_EXE_LINKER_FLAGS_COMMON "${FE_EXE_LINKER_FLAGS_COMMON} -Wl,-z,pack-relative-relocs")
	ENDIF(HAVE_DT_RELR)
ENDIF(UNIX AND NOT APPLE)

SET(FE_SHARED_LINKER_FLAGS_COMMON "${FE_EXE_LINKER_FLAGS_COMMON}")
SET(FE_MODULE_LINKER_FLAGS_COMMON "${FE_EXE_LINKER_FLAGS_COMMON}")
...
IF(NOT CMAKE_SYSTEM MATCHES "Linux")
	FOREACH(FLAG_REMOVE "--no-undefined" "--no-allow-shlib-undefined")
		STRING(REPLACE "-Wl,${FLAG_REMOVE}" "" FE_SHARED_LINKER_FLAGS_COMMON "${FE_SHARED_LINKER_FLAGS_COMMON}")
		STRING(REPLACE "-Wl,${FLAG_REMOVE}" "" FE_MODULE_LINKER_FLAGS_COMMON "${FE_MODULE_LINKER_FLAGS_COMMON}")
...
IF(ENABLE_WERROR)
	SET(FE_C_FLAGS_COMMON   "${FE_C_FLAGS_COMMON} -Werror")
	SET(FE_CXX_FLAGS_COMMON "${FE_CXX_FLAGS_COMMON} -Werror")
...
SET(FE_C_FLAGS_DEBUG		"${CFLAG_OPTIMIZE_DEBUG} -ggdb -DDEBUG -D_DEBUG")
SET(FE_CXX_FLAGS_DEBUG		"${CFLAG_OPTIMIZE_DEBUG} -ggdb -DDEBUG -D_DEBUG")

SET(FE_C_FLAGS_RELEASE		"-O2 -DNDEBUG ${CFLAGS_VECTORIZE}")
SET(FE_CXX_FLAGS_RELEASE	"-O2 -DNDEBUG ${CFLAGS_VECTORIZE}")

SET(FE_C_FLAGS_RELWITHDEBINFO	"-O2 -ggdb -DNDEBUG ${CFLAGS_VECTORIZE}")
SET(FE_CXX_FLAGS_RELWITHDEBINFO	"-O2 -ggdb -DNDEBUG ${CFLAGS_VECTORIZE}")
...
SET(FE_CXX_FLAGS_DEBUG "${FE_CXX_FLAGS_DEBUG} -D_GLIBCXX_ASSERTIONS -D_GLIBCXX_DEBUG -D_GLIBCXX_DEBUG_PEDANTIC")

# libc++ (clang only)
IF(CMAKE_CXX_COMPILER_ID STREQUAL "Clang")
	SET(FE_CXX_FLAGS_DEBUG "${FE_CXX_FLAGS_DEBUG} -D_LIBCPP_ASSERT=1 -D_LIBCPP_DEBUG=1 -D_LIBCPP_ENABLE_HARDENED_MODE=1")

	IF(CMAKE_CXX_COMPILER_VERSION VERSION_GREATER 16.50)
		# clang-17: Use _LIBCPP_HARDENING_MODE.
		SET(FE_CXX_FLAGS_DEBUG "${FE_CXX_FLAGS_DEBUG} -D_LIBCPP_HARDENING_MODE=_LIBCPP_HARDENING_MODE_DEBUG")
	ELSE(CMAKE_CXX_COMPILER_VERSION VERSION_GREATER 16.50)
		...
		SET(FE_CXX_FLAGS_DEBUG "${FE_CXX_FLAGS_DEBUG} -D_LIBCPP_ENABLE_ASSERTIONS=1")
...
IF(ENABLE_LTO)
	...
	IF(EXISTS "${GCC_WRAPPER_AR}" AND EXISTS "${GCC_WRAPPER_RANLIB}")
		...
		IF(CFLAG_LTO)
			SET(FE_C_FLAGS_RELEASE   "${FE_C_FLAGS_RELEASE} -flto")
			SET(FE_CXX_FLAGS_RELEASE "${FE_CXX_FLAGS_RELEASE} -flto")
			SET(FE_EXE_LINKER_FLAGS_RELEASE    "${FE_EXE_LINKER_FLAGS_RELEASE} -flto -fuse-linker-plugin")
			SET(FE_SHARED_LINKER_FLAGS_RELEASE "${FE_SHARED_LINKER_FLAGS_RELEASE} -flto -fuse-linker-plugin")
			SET(FE_MODULE_LINKER_FLAGS_RELEASE "${FE_MODULE_LINKER_FLAGS_RELEASE} -flto -fuse-linker-plugin")

			SET(FE_C_FLAGS_RELWITHDEBINFO   "${FE_C_FLAGS_RELWITHDEBINFO} -flto")
			SET(FE_CXX_FLAGS_RELWITHDEBINFO "${FE_CXX_FLAGS_RELWITHDEBINFO} -flto")
			SET(FE_EXE_LINKER_FLAGS_RELWITHDEBINFO    "${FE_EXE_LINKER_FLAGS_RELWITHDEBINFO} -flto -fuse-linker-plugin")
			SET(FE_SHARED_LINKER_FLAGS_RELWITHDEBINFO "${FE_SHARED_LINKER_FLAGS_RELWITHDEBINFO} -flto -fuse-linker-plugin")
			SET(FE_MODULE_LINKER_FLAGS_RELWITHDEBINFO "${FE_MODULE_LINKER_FLAGS_RELWITHDEBINFO} -flto -fuse-linker-plugin")
		ELSE(CFLAG_LTO)
